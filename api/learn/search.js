import { allowMethods, safeServerError } from '../../lib/api-utils.js';
...
async function searchYoutubeVideos(topic, language, query) {
  ...
  const videos = renderers
    .map((renderer) => {
      ...
      const channel = textFromRuns(renderer.ownerText) || textFromRuns(renderer.longBylineText);
      ...
      return {
        videoId,
        title,
        channel: channel || 'YouTube Chemistry',
        ...
        description,
        summary: summarizeVideo(topic, title, channel || 'YouTube Chemistry', language)
      };
    })
    .filter(Boolean)
...
export default async function handler(req, res) {
  ...
  try {
    const [primaryVideos, trending] = await Promise.all([
    ...
    const safeVideos = videos.length ? videos : buildFallbackVideos(topic, language);

    return res.status(200).json({
      topic,
      language: language.value,
      query: videos.length ? (primaryVideos.length ? query : relaxedQuery) : query,
      source: videos.length ? 'youtube' : 'fallback',
      notice: videos.length ? '' : 'Live YouTube results were limited, so fallback chemistry videos were returned.',
      videos: safeVideos,
