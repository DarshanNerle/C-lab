...
import { Clock3, Expand, Maximize2, Minimize, Minus, NotepadText, Play, ShieldAlert, SlidersHorizontal } from 'lucide-react';
...
export default function LabEnhancementsHUD({ contextKey = 'lab', reaction, onReplay }) {
    ...
    const handlePillPointerDown = (event) => {
        if (isMobileViewport || isExpanded) return;

        dragStateRef.current.active = true;
        dragStateRef.current.startX = event.clientX;
        dragStateRef.current.startY = event.clientY;
        dragStateRef.current.originX = pillOffset.x;
        dragStateRef.current.originY = pillOffset.y;
    };

    return (
        <>
            <div
                className={`pointer-events-auto z-[105] rounded-2xl border border-white/10 bg-slate-950/80 text-white shadow-xl backdrop-blur-xl transition-all duration-300 ease-in-out ${isMobileViewport
                    ? 'fixed left-1/2 bottom-16 w-[90vw] max-w-md -translate-x-1/2'
                    : 'absolute right-4 top-20 w-[300px] max-w-[calc(100%-2rem)]'
                    } ${isExpanded ? 'scale-100 opacity-100' : 'pointer-events-none scale-95 opacity-0'}`}
                style={{ transformOrigin: isMobileViewport ? 'bottom center' : 'top right' }}
            >
                <div className="p-4">
                    <div className="flex items-center justify-between text-xs uppercase tracking-[0.14em] text-slate-400">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-300 transition hover:border-cyan-400/40 hover:bg-white/10 hover:text-cyan-100"
                                aria-label="Minimize lab controls"
                            >
            ...
            <div
                className={`fixed z-[116] rounded-full border border-white/10 bg-slate-950/90 px-3 py-2 text-white shadow-xl backdrop-blur-xl transition-all duration-300 ease-in-out ${isExpanded ? 'pointer-events-none scale-95 opacity-0' : 'pointer-events-auto scale-100 opacity-100'} ${isMobileViewport ? 'left-1/2 bottom-4 -translate-x-1/2' : 'bottom-4 right-4'}`}
                style={!isMobileViewport ? { transform: `translate(${pillOffset.x}px, ${pillOffset.y}px)` } : undefined}
                onPointerDown={handlePillPointerDown}
            >
                <div className="flex items-center gap-3">
                    <span className="inline-flex items-center gap-1 text-xs font-mono text-cyan-200">
                    ...
                    <button
                        ...
                        className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/15 bg-white/10 text-slate-200 transition hover:border-cyan-400/40 hover:bg-white/20 hover:text-cyan-100"
                        aria-label="Expand lab controls"
                    >
            ...
            <div className={`pointer-events-auto absolute bottom-24 right-4 z-[115] w-[340px] max-w-[calc(100%-2rem)] rounded-2xl border border-white/10 bg-slate-950/90 p-3 text-white shadow-xl backdrop-blur-xl transition-all duration-300 ease-in-out ${notesOpen && isExpanded ? 'scale-100 opacity-100' : 'pointer-events-none scale-95 opacity-0'}`}>
            ...
            <div className={`fixed inset-x-3 bottom-3 z-[130] grid grid-cols-4 gap-2 rounded-2xl border border-white/15 bg-slate-950/90 p-2 text-xs backdrop-blur-xl transition-all duration-300 md:hidden ${isExpanded ? 'opacity-100 scale-100' : 'pointer-events-none opacity-0 scale-95'}`}>
                <button
                    type="button"
                    onClick={() => setNotesOpen((prev) => !prev)}
